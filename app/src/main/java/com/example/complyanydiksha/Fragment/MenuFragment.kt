...
import com.example.complyanydiksha.LawFragment
import com.example.complyanydiksha.ProxyUserFragment
import com.example.complyanydiksha.R
import com.example.complyanydiksha.SubLawFragment
import com.example.complyanydiksha.activity.HelpAndSupportActivity
...
//        backSubLawOrSubHead = view.findViewById(R.id.backSubLawOrSubHead)
        ...
        globalCompany.setOnClickListener {
            fragmentManager?.beginTransaction()?.replace(
                R.id.frame,
                GlobalCompanyFragment(), null
            )?.addToBackStack("tag")?.commit()
        }
        regionalCompany.setOnClickListener {
            fragmentManager?.beginTransaction()?.replace(
                R.id.frame,
                RegionalCompanyFragment(), null
            )?.addToBackStack("tag")?.commit()
        }
        subRegionalCompany.setOnClickListener {
            fragmentManager?.beginTransaction()?.replace(
                R.id.frame,
                SubRegionalCompanyFragement(), null
            )?.addToBackStack("tag")?.commit()
        }
        countryCompany.setOnClickListener {
            fragmentManager?.beginTransaction()?.replace(
                R.id.frame,
                CountryCompanyFragment(), null
            )?.addToBackStack("tag")?.commit()
        }
